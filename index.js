...
function MenuElemaniOlustur(isim, fiyat, kategori) {
  const kahvaltı = {
    ...
    menuyuVer: function () {
      const sonuc = `isim:" ${this.isim}" fiyat: ${this.fiyat} kategori: "${this.kategori}"`;
      return sonuc;
    },
  };
  return kahvaltı.menuyuVer();
}
...
const burger = {
  ...
  indirim: function (degisken) {
    if (degisken === "öğretmen" || degisken === "öğrenci") {
      const indirimliFiyat = this.fiyat - (this.fiyat * 25) / 100;
      return indirimliFiyat;
    } else {
      const indirimliFiyat = this.fiyat - (this.fiyat * 10) / 100;
      return indirimliFiyat;
    }
  },
};
...
*/
// const filter = degerlendirmeler.filter(item => item.isim === 'Ahmet');
// console.log(filter[0].geribildirim);
...
*/
const render = degerlendirmeler.filter((item) => item.isim === "Reyna");
render[0].geribildirim =
...
function DegerledirmeEkle(isim, puan, geribildirim) {
  const degerlendirme = {
    isim,
    puan,
    geribildirim,
  };
  const dizi = [...degerlendirmeler, degerlendirme];

  console.log(dizi);
  return dizi;
}
DegerledirmeEkle("Hurşut", 2, "Boktan yemekler!");
/*  Görev 6: 
...
function AnahtardanDegerlendirmeAl(dizi, sıra) {
  const yeniDizi = [...dizi];
  const istenilenDizi = dizi[sıra];
  const sonuc = `${istenilenDizi.isim} isimli kişi ${istenilenDizi.puan} puan  verdi ve şunları yazdı : ${istenilenDizi.geribildirim}`;
  return sonuc;
}
/*  Görev 7:  
...
function SonDegerlendirmeyiAl(sonuc) {
  const dizi = sonuc;
	const sonSıra = dizi[dizi.length - 1];
	const total = `${sonSıra.isim} isimli kişi  ${sonSıra.puan} verdi ve şunları söyledi  ${sonSıra.geribildirim}`;
	console.log(total);
	return total
}
